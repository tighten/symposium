...
<div class="container">
	...
	<ul class="list-styles">
		...
		<li><h3><a href="/styles/{{ $style->slug }}">{{ $style->title }}</a></h3>
			<p class="style-meta">By <a href="/authors/{{ $style->author->id }}">{{ $style->author->first_name }} {{ $style->author->last_name }}</a> - <i>{{ $style->created_at->toFormattedDateString()  }}</i> - popularity {{ round($style->visits / $total_visits * 100) }}</p></li>
		@endforeach
	</ul>
</div>
@stop
