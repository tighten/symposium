<nav :class="slotProps.showNav ? 'block' : 'hidden'" class="absolute left-0 items-center block w-full p-4 font-sans text-sm font-semibold text-indigo-800 bg-white shadow-sm lg:w-auto lg:relative lg:shadow-none lg:flex">
    @if (auth()->check())
        @php
            $menuItems = [
                [
                    'title' => 'Dashboard',
                    'url' => route('dashboard'),
                    'classes' => 'font-medium ml-0 lg:mr-4 sm:mx-2 md:mx-4',
                ],
                [
                    'title' => 'Bios',
                    'url' => route('bios.index'),
                    'classes' => 'font-medium lg:mr-4 sm:mx-2 md:mx-4 mt-2 lg:mt-0',
                ],
                [
                    'title' => 'Conferences',
                ...
                [
                    ...
                    'classes' => 'font-medium lg:mr-4 sm:ml-2 md:ml-4 mt-2 lg:mt-0',
                ],
        ...
        @php
            $menuItems = [
                ...
                [
                    'title' => 'Conferences',
                    'url' => route('conferences.index'),
                    'classes' => 'font-medium lg:mr-4 sm:mx-2 md:mx-4 sm:mr-0 mt-2 lg:mt-0',
                ],
            ];
        @endphp
    @endif
    ...
    @foreach ($menuItems as $item)
        <a class="block border-b border-indigo border-solid hover:text-indigo-500 py-5 lg:border-none uppercase {{ $item['classes'] }}"
           href="{{ $item['url'] }}"
        >
            {{ $item['title'] }}
        </a>
